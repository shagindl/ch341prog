...
/* This procedure involves
  1. Some custom timings of the 2-wire interface, processing the response bit of the IIC bus, and understanding the internal timing analysis of the 2-wire interface
  2. Provide example programs to operate 2-wire interface IIC devices X76F640, PCF8574, PCA9554
  3. Use EPP or MEM parallel port for multi-bit output or multi-bit input to simulate read-only or write-only SPI timing
  4. Use UIO general I/O bit stream commands to implement a custom synchronous serial interface
  5. Provide example programs to operate TLC1549, a non-standard serial timing device similar to SPI
  6. Provide example programs to operate 4-wire interface SPI devices 25C512, 25C020 through CH341StreamSPI4
  7. Provide an example program to operate a non-standard serial timing device ADC0831 similar to SPI through CH341BitStreamSPI
  In addition, you can use CH341SetOutput to set the I/O direction of CH341, and directly output data through any pin of CH341. No examples are provided. It is recommended to use CH341Set_D5_D0 instead
*/

/* The API level of CH341 parallel port driver and DLL is divided from low to high
...
/* Measured speed
   ...
   CH341WriteEEPROM   About 5K bytes (if it is RAM instead of flash memory, the speed is the same as CH341ReadEEPROM)
...
/* ********************************************************************************************** */
/* Example: universal operation timing compatible with IIC bus */
...
BOOL	WINAPI	IIC_OutByteCheckAck(  // Output one byte of data and check whether the response is valid
	ULONG			iIndex,  // Specify the serial number of the CH341 device
...
BOOL	WINAPI	IIC_InBlockByAck(  // Input data block, each byte input will generate a valid response
	ULONG			iIndex,  // Specify the serial number of the CH341 device
	ULONG			iInLength,  // The number of data bytes to be read must be less than 32 bytes at a time
	PVOID			oInBuffer )  // Point to a buffer, after returning it is the data read
{
	UCHAR	mBuffer[ mCH341_PACKET_LENGTH ];
	ULONG	mLength, mInLen;
...
BOOL	WINAPI	IIC_InByteNoAck(  // Input one byte of data, but no response
	ULONG			iIndex,  // Specify the serial number of the CH341 device
	PUCHAR			oInByte )  // Point to a byte buffer, after the return is the read data
{
	UCHAR	mBuffer[ mCH341_PACKET_LENGTH ];
	ULONG	mLength, mInLen;
	mBuffer[ 0 ] = mCH341A_CMD_I2C_STREAM;  // 命令码
...
BOOL	WINAPI	PCF8574_WriteIO(  // Выход PCF8574 I / O
	ULONG			iIndex,  // Specify the serial number of the CH341 device
	ULONG			iDeviceAddr,  // Адрес устройства, младший бит - это бит направления команды
	ULONG			iOutByte )  // Данные ввода / вывода готовы к записи
{  // 可以直接用CH341StreamI2C( iIndex, 2, mBuffer, 0, NULL )实现
	UCHAR	mBuffer[ mCH341_PACKET_LENGTH ];
	ULONG	mLength;
	mBuffer[ 0 ] = mCH341A_CMD_I2C_STREAM;  // Код команды
	mBuffer[ 1 ] = mCH341A_CMD_I2C_STM_STA;  // Сгенерировать стартовый бит
	mBuffer[ 2 ] = (UCHAR)( mCH341A_CMD_I2C_STM_OUT | 2 );  // Выходные данные, бит 5 бит 0 - длина, 2 байта
	mBuffer[ 3 ] = (UCHAR)( iDeviceAddr & 0xFE );  // Device address, write operation
	mBuffer[ 4 ] = (UCHAR)iOutByte;  // Number of I / O
	mBuffer[ 5 ] = mCH341A_CMD_I2C_STM_STO;  // Stop bit
	mBuffer[ 6 ] = mCH341A_CMD_I2C_STM_END;  // The current package ends early
	mLength = 7;
	return( CH341WriteData( iIndex, mBuffer, &mLength ) );  // 写出数据块
}
